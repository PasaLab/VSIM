...
import cn.edu.nju.pasalab.graph.vsim.{ResultPair, VertexRecord}
...
object TestThreadLevelBalance {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val taskDS = spark.read.parquet(options.taskFile).as[VertexRecord]
    val v = taskDS.count()
    val numVerticesPerPartition = (v / options.numPartitions).toInt
    val onePartitionData = taskDS.rdd.takeSample(false, numVerticesPerPartition)
    val t1 = System.nanoTime()
    Thread.sleep(15000)
    System.err.println(s"Got one partition data of ${onePartitionData.length} records.")
    val pool = WorkerThreadPool.getForkJoinPool(options.threadNumPerExecutor)
    val taskSupport = new ForkJoinTaskSupport(pool)
    def executeTasks(tasks:Seq[VertexRecord]): Double = {
      val scTasks = tasks.par
      val graphStore = SimpleGraphStore.getSingleton
      scTasks.tasksupport = taskSupport
      val t0 = System.nanoTime()
      scTasks.foreach(record => processVertex(record.vid, record.adj, options, graphStore, accumulators))
      val t1 = System.nanoTime()
      (t1 - t0) / 1e9
    }
    val tRandom = executeTasks(scala.util.Random.shuffle(onePartitionData.toList))
    System.err.println("Random order executed.")
    Thread.sleep(10000)
    val tSorted = executeTasks(onePartitionData.toList.sortBy(r => -r.adj.length))
    println(s"Elapsed time of the random order: $tRandom s.")
    println(s"Elapsed time of the sorted order: $tSorted s.")
    spark.stop()
    Thread.sleep(10000)
    System.exit(0)
  }
