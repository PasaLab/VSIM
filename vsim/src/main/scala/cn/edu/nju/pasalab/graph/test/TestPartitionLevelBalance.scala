...
import cn.edu.nju.pasalab.graph.util.{HDFSUtils, ProcessLevelSingletonManager}
...
object TestPartitionLevelBalance {

  def main(args: Array[String]): Unit = {
    println("VSIM starts." + new Date())
    val options = VSIMOptions(args)
    println("Get options: " + options.toString)
    val spark = SparkSession.builder().appName(s"VSIM (PartitionLevelBalance): ${options.input}-${options.threshold}")
    ...
    val stats = taskDS.rdd.mapPartitions(recordIterator => {
    ...
    stats.saveAsTextFile(options.outputPath)
  ...
  def processVertex(vid: Long, adj: Array[Long], options: VSIMOptions,
                    graphStore: SimpleGraphStore, accumulators: Accumulators): Seq[String] = {
    val t0 = System.nanoTime()
    ...
    val t1 = System.nanoTime()
    Array(s"$vid,${adj.length},${(t1 - t0) / 1e6}")
  }
